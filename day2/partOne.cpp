...
int main() {
  std::ifstream inputFile("input_em.txt");
  std::string line;
  ...
  if (inputFile.is_open()) {
    while(getline(inputFile, line)) {
      ...
      int policyEvaluationCounter = 0;
      for (auto it = password.begin(); it != password.end(); ++it) {
        if (*it == policyLetter) {
          policyEvaluationCounter++;
        }
      }
      if (policyEvaluationCounter >= lowest && policyEvaluationCounter <= highest) {
